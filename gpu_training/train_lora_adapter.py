...
EPOCHS = 50
BATCH_SIZE = 16
LR = 2e-4               # Higher LR is fine for LoRA (only small params update)
WEIGHT_DECAY = 0.01
WARMUP_RATIO = 0.1
...
def download_afdb():
    """Download MIT-BIH AF Database."""
    afdb_dir = os.path.join(DATA_DIR, "afdb")
    if os.path.exists(afdb_dir) and len([f for f in os.listdir(afdb_dir) if f.endswith(".dat")]) >= 20:
        logger.info(f"AFDB already downloaded at {afdb_dir}")
        return afdb_dir
    os.makedirs(afdb_dir, exist_ok=True)
    logger.info("Downloading MIT-BIH AF Database (~25 records, 10-hour each)...")
...
def train():
    ...
    for epoch in range(EPOCHS):
        ...
        if (epoch + 1) % 5 == 0 or epoch == 0 or val_f1 > best_val_f1:
            logger.info(
